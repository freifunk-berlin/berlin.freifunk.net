...
* Wireless Network Protection should be `on`. This protects airMAX networks against de-authentication attacks.
...
* Multicast-Enhancement must be `off`. Without this babeld will not work properly due to issues with IPv6 multicast groups.
...
Disable them as they mask issues that should be properly fixed! In addition if the duration is too short a watchdog will cause a firmware update to fail and result in a bricked device.
